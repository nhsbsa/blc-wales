...
router.post(/where-do-you-live/, (req, res) => {
  const country = req.session.data['whereDoYouLive'];
  if (country == 'other') {
      res.redirect('/wales-discovery/location-kickout-uk');
  } else {
     res.redirect('/wales-discovery/relation-to-baby');
  }
})
...
router.post(/triplets-loss-2/, (req, res) => {
  ...
  if (tripletLoss == '1-cert') {
    res.redirect('date-of-loss-1-cert');
